...
-- SELECT DATE_ADD(`Check`, INTERVAL 1 DAY)
-- FROM `b'test2'`.omronprojectpdfstranslated
-- WHERE `Period` = 'Daily';

-- Adding 3 Months to Date
-- SELECT DISTINCT `Robot`,`Part`, DATE_ADD(`Check`, INTERVAL 90 DAY) AS 90DayDiff
-- FROM `b'test2'`.omronprojectpdfstranslated
-- WHERE `Period` = '3 Months';

-- Adding 6 Months to Date
-- SELECT DATE_ADD(`Check`, INTERVAL 180 DAY) AS 180DayDiff
-- FROM `b'test2'`.omronprojectpdfstranslated
-- WHERE `Period` = '6 Months';

-- Adding 1 Year to Date
SELECT DATE_ADD(`Check`, INTERVAL 365 DAY) AS 365DayDiff
FROM `b'test2'`.omronprojectpdfstranslated
WHERE `Period` = 'Yearly';
